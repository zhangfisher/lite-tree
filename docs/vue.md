...
`LiteTree`允许为节点设置样式，节点的标题、标识、标签、注释均可以指定样式。

只需要在节点的标题、标记、标签、注释前置声明`{css样式}`即可，如`{color:red}xxxxxxx`。


```vue
<LiteTree>
我的项目
    {color:blue}VoerkaI18n  //  
    speedform                           //={color:green}
    Logsets                             //  {color:red}终端增强
    VoerkaLogger({color:red;border:1px solid red;}Log)      // 标签
</LiteTree>
```
...
`LiteTree`对标准的`{css样式}`做了少量扩展，支持在样式中指定变量或样式类，如`{#border;.primary;color:red;}`的形式。

示例如下：
```ts
<LiteTree>
#border=border:1px solid red;
.primary=background-color:#009800;
---
我的项目
    {#border;.primary;color:blue}VoerkaI18n    
</LiteTree>
```
<LiteTree>
#border=border:1px solid red;
